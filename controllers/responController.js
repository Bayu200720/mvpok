...
responRouter.use(bodyParser.urlencoded({ extended: false }));
responRouter.use(bodyParser.json());

//POST Response role CS 
    ...
    jwt.verify(token, Conf.secret, async function(err, decoded) {
        ...
        if(cs && cs.length !== 0){
            try {
                ...
                const { detail_respon, id_tag } = req.body;
                ...
                await respon.save();
                res.status(201).send({ respon, updateDataComplaint });
            } catch (err) {
...
//Update tag by id respon role CS 
        ...
        if(cs && cs.length !== 0){
            try {
                ...
                if (complaint) {
                    ...
                    res.status(201).send({ updateDataComplaint, updateDataRespon });
                } else {
                    res.status(404).json({
        ...
        if(customer && customer.length !== 0){
            try {
                ...
                if (complaint) {
                    ...
                    const updateDataRespon = await respon.save();
                    res.status(201).send({ updateDataRespon, updateDataComplaint });
                } else {
                    res.status(404).json({
                        message: 'Complaint not found'
                    })
...
responRouter.get('/get-respon-by-cs', async (req,res) => {
    ...
    jwt.verify(token, Conf.secret, async function(err, decoded) {
        ...
        const cs = await CS.findById(decoded.id);
        if(cs && cs.length !== 0){
            const respon =  await Respon.find({"id_cs": cs._id});
            if(respon && respon.length !== 0) {
                res.json(respon)
            } else {
                res.status(404).json({
                    ...
                    [
                      {
                        $group:
                        {
                            _id:"$id_cs",
                            count:{$sum:1},
                            total : {$sum:{$multiply:["$rating"]}},
                            average:  {$avg:{$multiply:["$rating"]}},
                        }
                      }
                    ]
                )
